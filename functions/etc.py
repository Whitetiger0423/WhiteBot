...
class etc(commands.Cog):
    ...
    async def code(self, ctx, type: Option(str, "암호화 시킬 방식을 선택하세요", choices=["아스키 코드", "base64"]), text):
        if type == "아스키 코드":
            ...
            embed = discord.Embed(title="<a:check:824251178493411368> 암호화 완료!", description="base64를 기반으로 한 암호문입니다.", color=0xffffff)
            embed.add_field(name="**암호문:**", value=f"```{data}```", inline=False)
            await ctx.respond(embed=embed)

    @slash_command(description='수신문을 해독합니다.')
    async def decode(self, ctx, type: Option(str, "해독할 암호문의 암호화 방식을 선택하세요", choices=["아스키 코드", "base64"]), text):
        if type == "아스키 코드":
            data = decrypt(text)
            try:
