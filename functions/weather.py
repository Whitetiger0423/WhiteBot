...
class Weather(commands.Cog):
    ...
    async def weather(self, ctx, place: Option(str, "검색할 지역을 입력하세요.", name="장소")):
        ...
        async with self.session.get(url) as location_response:
            json = await location_response.json()
            if len(json):
                location = json[0]
                ...
                async with self.session.get(url) as weather_response:
                    json = await weather_response.json()
                    ...
                    embed.add_field(name="온도 최대/최저",
                                    value=f"{json['main']['temp_max']}°C / {json['main']['temp_min']}°C")
