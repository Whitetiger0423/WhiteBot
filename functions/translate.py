    ...
    def papago_translate(
        ...
        if res.status_code == 200:
            ...
            if body["errorCode"] == "010":
                ...
                return discord.Embed(title="오류 발생", description=err_msg, color=0xFF0000)

    def google_translate(
        self, src_lang: str, tar_lang: str, text: str
    ) -> discord.Embed:
        data = json.dumps(
            {"q": text, "source": src_lang, "target": tar_lang, "format": "text"}
        )
        header = {
        ...
        if res.ok:
            ...
            return discord.Embed(
                title="번역 완료", description=result, color=0x00FFC6
            ...
            print("Response:", res.text)
            return discord.Embed(
                title="오류 발생", description="오류가 발생했어요. 잠시 후에 다시 시도해주세요", color=0xFF0000
            )
